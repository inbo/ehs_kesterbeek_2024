f.map <- function(data,X,Y,crs,control.parameter,label.parameter,plot=FALSE){
  ...
  RangePlot<-leaflet()%>%
    addTiles()%>%
    addCircleMarkers(data=coords_SP,radius=8,fillColor = ~beatCol(coords_SP@data[,control.parameter]),fillOpacity=0.8,weight=1,color='black',popup=Samples_text,label=coords_SP@data[,label.parameter],labelOptions = labelOptions(noHide = T),group=coords_SP@data[,control.parameter])%>%
